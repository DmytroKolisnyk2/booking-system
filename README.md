...
На фото вище ви можете побачити структуру папки `helpers` що виконує роль Api проєкту, в ній є всі запити на бекенд що використовуються в нашому проєкті, всі вони мають однакову структуру, тому ми розберомся з найлегчою категорією запитів - `course`. На фото нижче ви можете детальніше побачити структуру наших запитів 🎯:


Ми використовуємо бібліотеку axios для полегчення роботи з бекендом, також в кожному файлі в основному чотири типи запитів - `get`, `post`, `put`, `delete`, зараз ми розберемо на прикладі запит getCourses():

Коли ми його викликаємо ти можемо побачити що вона робить запит на дефолтний url, тобто нашого бекенду, з роутом `/courses`, і далі просто повертає всі дані що запит витягнув з бека, на фронті ми можемо побачити наступну ситуацію:


Ну і потім в компонентах ми можемо відрендерити дані що отримали з бекенду
