...
Тобто папка-ім'я сторінки, сама сторінка, і в якості стилів scss модулі
Розберемо на прикладі сторінки `caller` - користувача що назначає зустрічі на менеджера 👩‍💼: 



тут ми бачимо імпорт компонентів для використання, таких як [table і table item](https://reactjs.org/docs/hooks-intro.html) та інших, також імпорт запитів і `caller-selectors, caller-operations`, а також [хуки](https://reactjs.org/docs/hooks-intro.html) `useState`, `useEffect`, далі починається код самої сторінки 👨‍💻:
...
Ми використовуємо заздалегідь підготовані компоненти [table](https://reactjs.org/docs/hooks-intro.html),datepicker а також days, для відображення днів, все найцікавіше починається в самому компоненті таблиці, але про нього пізніше. Таким чином побудовані всі сторінку нашого сайту, окрім `superadmin`, бо для неї id користувача не потрібно.
